...
export function PolicyChatPanel({ userRole, policies }: Props) {
  const initialMsg = POLICY_CHAT_INITIAL_MESSAGES[userRole] ?? POLICY_CHAT_INITIAL_MESSAGES.requester
  ...
  async function send() {
    ...
    try {
      const sections  = retrievePolicySections(text, policies)
      const apiHistory = nextMessages.slice(1) as PolicyChatMessage[]
      let full = ''
      for await (const token of streamPolicyChat(userRole, apiHistory, sections)) {
        full += token
        setStreamText(full)
      }
    ...
    <div className="ai-surface" style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
      ...
      <div style={{
        ...
        <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--violet-700)' }}>
          {ROLE_TITLES[userRole] ?? 'Policy Assist'}
        </span>
      ...
      <div style={{
        ...
        <input
          ...
          placeholder="Ask about PDPL policies…"
          style={{ flex: 1, height: 32, fontSize: 13 }}
          disabled={streaming}
          aria-label="Ask the policy assistant"
        />
        <button
